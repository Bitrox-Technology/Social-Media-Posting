...
import { ProductBenefitTemplates, ProductBenefitTemplate, ProductBenefitSlide } from '../../templetes/Informative/productBenefit';
import { IndustryTrendTemplates, IndustryTrendTemplate, IndustryTrendSlide } from '../../templetes/Informative/industryTrends';
import { TipsTemplates, TipsTemplate, TipsSlide } from '../../templetes/Informative/Tips';
...
type InformativeTemplate = ProductBenefitTemplate | IndustryTrendTemplate | TipsTemplate;

// Type guards to narrow down the template type
const isProductBenefitTemplate = (template: InformativeTemplate): template is ProductBenefitTemplate => {
    return template.id.includes('product-benefit');
};

const isIndustryTrendTemplate = (template: InformativeTemplate): template is IndustryTrendTemplate => {
    return template.id.includes('industry-trend');
};

const isTipsTemplate = (template: InformativeTemplate): template is TipsTemplate => {
    return template.id.includes('tips');
};

export const InformativeTemplateSelector: React.FC = () => {
    const navigate = useNavigate();
    const { theme } = useTheme();
    const defaultLogoUrl = '/images/Logo.png';
    const [selectedTemplate, setSelectedTemplate] = useState<InformativeTemplate | null>(null);
    const [activeTab, setActiveTab] = useState<'productBenefits' | 'industryTrends' | 'tips'>('productBenefits');
    ...
    const handleSelectTemplate = (template: InformativeTemplate) => {
    ...
    const templates = {
        productBenefits: ProductBenefitTemplates,
        industryTrends: IndustryTrendTemplates,
        tips: TipsTemplates,
    };
    ...
    const renderPreview = () => {
        ...
        if (isProductBenefitTemplate(selectedTemplate)) {
            const slide = selectedTemplate.slides[0] as ProductBenefitSlide;
        ...
        if (isIndustryTrendTemplate(selectedTemplate)) {
            const slide = selectedTemplate.slides[0] as IndustryTrendSlide;
            return selectedTemplate.renderSlide(
                {
                    ...slide,
        ...
        if (isTipsTemplate(selectedTemplate)) {
            const slide = selectedTemplate.slides[0] as TipsSlide;
            return selectedTemplate.renderSlide(
                {
                    ...slide,
                    imageUrl: slide.imageUrl ?? '',
            ...
            <div className="flex h-screen">
                ...
                >
                    ...
                    <div className="flex flex-wrap gap-2 mb-6">
                        <button
                            onClick={() => setActiveTab('productBenefits')}
                            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                                activeTab === 'productBenefits'
