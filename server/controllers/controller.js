...
const Content = async (req, res, next) => {
  ...
  try {
    ...
    if (thinkStart !== -1 && thinkEnd !== -1) {
      cleanedContent = generatedContent.slice(0, thinkStart) + generatedContent.slice(thinkEnd + 8);
    }
    cleanedContent = cleanedContent.trim();

    // Extract title
    const titleMatch = cleanedContent.match(/\*\*Title:\*\*\s*(.+)/i);
    const title = titleMatch ? titleMatch[1].trim() : `5 Innovative Ideas for ${topic}`;
    ...
    let contentStart = titleMatch ? cleanedContent.indexOf(title) + title.length : 0;
    let contentEnd = hashtagMatch ? cleanedContent.lastIndexOf(hashtagMatch[hashtagMatch.length - 1]) : cleanedContent.length;
    ...
    return res.status(200).json(new ApiResponse(200, contentResponse, 'Content generated successfully!'));
  } catch (error) {
  ...
  try {
    ...
    for (const idea of finalResponses) {
      if (!idea.title || !idea.content || !Array.isArray(idea.hashtags)) {
        ...
        idea.content = idea.content || `Explore how ${topic} drives innovation with this fresh idea!`;
        idea.hashtags = idea.hashtags || ['#AI', '#Innovation', '#Business'];
      }
    }

    // Return the array of content responses
    return res.status(200).json(new ApiResponse(200, finalResponses, 'Content ideas generated successfully!'));
  } catch (error) {
    console.error('Error generating content:', error.message);
    next(error);
  }
